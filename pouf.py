...
def main():
    ...
    while True:
        ...
        if 'state' in zone:
            title_text(draw, margin, device.width, "%s - %s" %
                       (zone['display_name'], state))

        if current:
            draw.text((margin, 50), text="%s / %s" % (str(datetime.timedelta(
                seconds=current)), str(datetime.timedelta(seconds=length))))
